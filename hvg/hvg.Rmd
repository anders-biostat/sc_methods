...
noR <- ScaleData(s, genes.use = rownames(s@data), vars.to.regress = NULL)
R   <- ScaleData(s, genes.use = rownames(s@data), vars.to.regress = c("nUMI", "percent.mito"))
...
## Compute PCA:
```{r}
pcFew  <- prcomp((t(normFew)), center = TRUE, scale. = FALSE)

pc_seurat_norm <- prcomp(t(noR@scale.data),
                         center = TRUE , scale. = FALSE)
pc_seurat_regr <- prcomp(t(  R@scale.data),
                         center = TRUE , scale. = FALSE)

# for even library sizes:
pceven<- prcomp((t(normeven)), center = TRUE, scale. = FALSE)
```
...
correctNeighbors <- function(prcompObj = NULL,
  ...
  sqDs <- matrix(0, nrow = nrow(prcompObj$x), ncol = ncol(prcompObj$x))    
  results <- tibble()
...
```{r message=FALSE}
tbl_1 <- correctNeighbors(pc_seurat_norm , ct, 31)
```
