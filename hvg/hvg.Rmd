...
# cell 251 is NN of 499 other cells! (found with: `which.max( table(nns) )

cellA <- 489 # popular neighbor
cellB <- 252
cellC <- 454 
 
pairs(normC[, c(cellA, cellB, cellC)], pch = 20, asp = 1,
      labels = c(cellA, cellB, cellC))

pc_corr_libsize <- 1 + (apply(pcFew$x, 2, function(pc) cor(pc, cs)) > .1)
pc_first20      <- c(rep("red", 20), rep("black", 480))

pairs( t(pcFew$x[c(cellA, cellB, cellC), ]), pch = 20,
      labels = c(cellA, cellB, cellC), asp = 1,
      col = pc_first20,
      cex = pcFew$sdev / 2)
```
...
### 250+250, even libsizes
Let's sample down CD4 T cells so that their library sizes are really
similar to those of CD8 T cells.

```{r}
cs <- colSums(rawC)
# we mask all non-CD4 cells by adding huge numbers:
dummycs <- cs; dummycs[! isCD4Tcell ] <- dummycs[ !isCD4Tcell] + 32000
ids <- sapply(cs[isCD8Tcell], function(ls) which.min( abs(ls - dummycs) ) )

selecteven <- c(ids, which(isCD8Tcell))
cteven     <- c( rep("CD4", 250),  rep("CD8", 250))
