...
# compute PCA on (almost) all genes (minus exremely noisy ones, for speed):
  ...
  pc  <- prcomp((t(norm2[ coarse_pick, ])), center = TRUE, scale. = TRUE)
  # I scale and center here but scaling is debatable.
  # Note that irlba's implementation in Seurat does the same.
...
sqDeviation <- function(PCvector) {
 ...
 sqDs <- matrix(0, nrow = 3018, ncol = 3018)    
 T_DF <- tibble()
for(p in 1:1000) {
  print(p)
...
ggplot(T_DF) + geom_point(aes(PCs, Mean_correctNeighbors, colour = Celltype)) +
  ggtitle("2768 CD4 T cells, 250 CD8 T cells")
```
This picture is nice, but not very clear yet as there are roughly 10x more CD4 
than CD8 T cells. So without any transcriptomic differences, we'd expect the
red line around 29 and the blue line around 1 - not sure why the red is
so far below and the blue so far above their expectations. Let's get some clarity
by evening the groups first (see next section).
...
# compute PCA on (almost) all genes (minus exremely noisy ones, for speed):
  coarse_pickFew <- rownames( vmrFew[vmrFew$log_Gene_VMR > vmrFew$log_PoissonVMR_median,] )
  
  
  pcFew  <- prcomp((t(normFew[ coarse_pickFew, ])), center = TRUE, scale. = TRUE)
  
```


### More PCs, less resolution
...
ggplot(few_DF) + geom_point(aes(PCs, Mean_correctNeighbors, colour = Celltype))+
  ggtitle("250 CD4, 250 CD8 T cells\nAll genes -> PCA -> eucl. distance on n PCs")
