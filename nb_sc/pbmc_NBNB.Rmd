...
```{r}
disptable <- pbmclapply(rownames(rawC)[1:10], function(g) {
...
k  = rawC["CD3E", ]
mu = 1.08175486
plot(seq(1,1.4, by=0.04), sapply(seq(1,1.4, by=0.04), function(d) sum( lgamma( k + 1/d ) - lgamma( 1/d ) - lgamma( k+1 ) - ( k + 1/d ) * log( 1 + sf * d * mu ) + k * log( sf * d * mu ) )), ylab = "likelihood", xlab = "Dispersion")
```
